...
summary(frenly$x[frenly$x==1963.75],frenly$y[frenly$x==1963.75])

data1<-data.frame(x=c(frenly$x[frenly$x > 1963 & frenly$x < 1970]), y=c(frenly$y[frenly$x > 1963 & frenly$x < 1970]))
data1

data2<-data.frame(x=c(frenly$x[frenly$x== 1964.50]), y=c(frenly$y[frenly$x==1964.50]))
data2

summary(data1,na.rm = TRUE)

boxplot(frenly$y,frenly$lag.quarterly.revenue,main='X vs Y',xlab = "X", ylab = "Y")
@
\textbf{Graficar el Type Quebec VS Treatment chilled.}
